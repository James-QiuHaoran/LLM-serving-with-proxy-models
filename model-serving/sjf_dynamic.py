...
class SJFWithDynamicBatching:
    ...
    def run(self, job_list):
        ...
        while curr_job_idx < len(job_list)-1 or len(wait_queue) > 0:
            ...
            # get the job_duration based on the running_batch
            # job_duration = max([x.get_execution_duration() for x in curr_batch])  # the longest job duration in the batch
            job_duration = min([x.get_execution_duration() for x in curr_batch])  # the shortest job duration in the batch
            ...
            # job completed
            for job in curr_batch:
                if job.get_execution_duration() <= job_duration_to_run:
                    ...
                    job.set_execution_duration(max(job.get_execution_duration() - job_duration_to_run, 0))
...
if __name__ == '__main__':
    ...
    scheduler = SJFWithDynamicBatching()
